...
import re
from AeroHelper.utils.icao import extract_icao_and_bearing, is_junk_icao_token
...
class OCRParser:
    ...
    def __init__(self):
        ...
        self.patterns = {
            ...
            'heading_before': re.compile(
                r'(?<!\d)(\d{1,3})\s+(?:HDG|HOG|H0G|TRK)',
                re.IGNORECASE,
            ),
        }
    ...
    def parse(self, text, preferred_code=None, prefer_custom=False, previous_distance=None):
        ...
        if not heading_matches:
            heading_matches = list(self.patterns['heading_before'].finditer(normalized_text))
        if heading_matches:
            try:

                chosen = None
                for m in heading_matches:
                    blob = m.group(0).upper()
                    if 'HDG' in blob or 'HOG' in blob or 'H0G' in blob:
                        chosen = m
                if chosen is None:
        ...
        if icao_code and bearing is not None and not is_junk_icao_token(icao_code):
