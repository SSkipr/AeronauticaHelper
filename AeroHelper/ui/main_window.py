...
HELPER_ICON_STYLE = f'color: {TEXT_SUBTLE}; padding: 0 2px;'
TOOLTIP_STYLE = f'''
    QToolTip {{
        color: {TEXT_PRIMARY};
...
CUSTOM_WAYPOINT_TOOLTIP = 'When checked: disables the stop/alert when the vehicle moves away from the destination. Use this when following a custom waypoint that may route around land masses.'
...
class MainWindow(QMainWindow):
    ...
    def _create_helper_icon(self, tooltip):
        icon = QLabel('ⓘ')
        icon.setFont(QFont(APP_FONT_FAMILY, scale_font(10)))
        icon.setCursor(Qt.PointingHandCursor)
        icon.setToolTip(tooltip)
        icon.setStyleSheet(HELPER_ICON_STYLE)
        return icon
    ...
    def _build_brand_header(self, layout):
        ...
        self.local_version_label = QLabel(local_ver_text)
        self.local_version_label.setFont(QFont(APP_FONT_FAMILY, scale_font(9)))
        self.local_version_label.setStyleSheet(PILL_STYLE)
        version_row.addWidget(self.local_version_label)
        self.version_check_label = QLabel('')
        self.version_check_label.setFont(QFont(APP_FONT_FAMILY, scale_font(9), QFont.Bold))
        self.version_check_label.setStyleSheet(PILL_STYLE)
    ...
    def _build_mode_section(self, layout):
        ...
        for mode_name in MODE_OPTIONS:
            ...
            mode_boxes_layout.addWidget(btn)
    ...
    def _build_notification_mode_row(self, layout):
        ...
        self.notification_mode_combo.addItems(['All Notifications', 'Urgent Only'])
        ...
        self.notification_mode_label.hide()
        self.notification_mode_combo.hide()
    ...
    def _build_option_checkboxes(self, layout):
        ...
        self.custom_waypoint_row = QHBoxLayout()
        ...
        self.custom_waypoint_checkbox.setToolTip(CUSTOM_WAYPOINT_TOOLTIP)
        self.custom_waypoint_info = QLabel('ⓘ')
        self.custom_waypoint_info.setFont(QFont(APP_FONT_FAMILY, 9))
        self.custom_waypoint_info.setCursor(Qt.PointingHandCursor)
        self.custom_waypoint_info.setToolTip(CUSTOM_WAYPOINT_TOOLTIP)
        self.custom_waypoint_info.setStyleSheet(HELPER_ICON_STYLE)
        self.custom_waypoint_row.addWidget(self.custom_waypoint_checkbox)
        self.custom_waypoint_row.addWidget(self.custom_waypoint_info)
        self.custom_waypoint_row.addStretch()
        layout.addLayout(self.custom_waypoint_row)
        self.custom_waypoint_checkbox.hide()
        self.custom_waypoint_info.hide()
        self.skip_bearing_row = QHBoxLayout()
        ...
        self.skip_bearing_checkbox.setToolTip(SKIP_CURRENT_BEARING_TOOLTIP)
        self.skip_bearing_info = QLabel('ⓘ')
        self.skip_bearing_info.setFont(QFont(APP_FONT_FAMILY, 9))
        self.skip_bearing_info.setCursor(Qt.PointingHandCursor)
        self.skip_bearing_info.setToolTip(SKIP_CURRENT_BEARING_TOOLTIP)
        self.skip_bearing_info.setStyleSheet(f'color: {WARNING_TEXT}; padding: 0 2px;')
        self.skip_bearing_row.addWidget(self.skip_bearing_checkbox)
        self.skip_bearing_row.addWidget(self.skip_bearing_info)
        self.skip_bearing_row.addStretch()
        layout.addLayout(self.skip_bearing_row)
        self.skip_bearing_checkbox.hide()
        self.skip_bearing_info.hide()
        self.throttle_up_row = QHBoxLayout()
        ...
        self.throttle_up_checkbox.setToolTip(THROTTLE_UP_TOOLTIP)
        self.throttle_up_info = QLabel('ⓘ')
        self.throttle_up_info.setFont(QFont(APP_FONT_FAMILY, 9))
        self.throttle_up_info.setCursor(Qt.PointingHandCursor)
        self.throttle_up_info.setToolTip(THROTTLE_UP_TOOLTIP)
        self.throttle_up_info.setStyleSheet(HELPER_ICON_STYLE)
        self.throttle_up_row.addWidget(self.throttle_up_checkbox)
        self.throttle_up_row.addWidget(self.throttle_up_info)
        self.throttle_up_row.addStretch()
        layout.addLayout(self.throttle_up_row)
        self.throttle_up_checkbox.hide()
        self.throttle_up_info.hide()
        self.quit_after_5_errors_row = QHBoxLayout()
        self.quit_after_5_errors_checkbox = QCheckBox('Quit after 5 consecutive errors (recommended)')
        ...
        self.quit_after_5_errors_checkbox.setToolTip(QUIT_AFTER_5_ERRORS_TOOLTIP)
        self.quit_after_5_errors_info = QLabel('ⓘ')
        self.quit_after_5_errors_info.setFont(QFont(APP_FONT_FAMILY, 9))
        self.quit_after_5_errors_info.setCursor(Qt.PointingHandCursor)
        self.quit_after_5_errors_info.setToolTip(QUIT_AFTER_5_ERRORS_TOOLTIP)
        self.quit_after_5_errors_info.setStyleSheet(f'color: {WARNING_TEXT}; padding: 0 2px;')
        self.quit_after_5_errors_row.addWidget(self.quit_after_5_errors_checkbox)
        self.quit_after_5_errors_row.addWidget(self.quit_after_5_errors_info)
        self.quit_after_5_errors_row.addStretch()
        layout.addLayout(self.quit_after_5_errors_row)
        ...
        self.include_screenshots_checkbox.setToolTip('When checked: attaches a screenshot to each mission status webhook update')
        ...
        self.share_data_checkbox.setToolTip('Sends diagnostic logs on errors so the developer can help you.\nOnly your webhook URL and the last 25 KB of AeroHelper.log are shared.')
        ...
        self.share_data_override_btn.setToolTip('Send your webhook URL and the last 25 KB of AeroHelper.log to the developer once, right now.\nDoes not require automatic sharing to be on.')
    ...
    def _build_info_panels(self, layout):
        self.monitoring_info = self._create_info_label('Keeps an eye on your mission while you play or AFK.\n\n• Sends Discord updates about speed, fuel, distance, and alerts\n• Choose All Notifications or Urgent Only\n• Presses 5 each cycle to read current heading (HDG) unless Unlock 5 view is on')
        layout.addWidget(self.monitoring_info)
        self.autosteer_info = self._create_info_label('Helps steer your vehicle toward the destination during a mission.\n\n• You still spawn and start missions yourself\n• AeroHelper keeps you on course toward DEST or your current target\n• Custom waypoint: use when your route temporarily moves away from destination\n• Multiplier adjusts how long steering is held; 1.0 is a good Seawise starting point')
        layout.addWidget(self.autosteer_info)
        self.autopilot_info = self._create_info_label('Runs the whole loop for you - lobby to finish and back again.\n\n• Finds jobs, picks routes, steers, docks, and repeats\n• Only for supported boats and airships (AFK gameplay loop)\n• Start from the lobby menu with a vehicle that can reach all destinations')
    ...
    def _build_cycle_controls(self, layout):
        ...
        self.interval_lock_icon = QLabel('Locked')
        self.interval_lock_icon.setFont(QFont(APP_FONT_FAMILY, scale_font(9), QFont.Bold))
        self.interval_lock_icon.setToolTip('Locked: AutoPilot uses 15s cycle delay')
        self.interval_lock_icon.setStyleSheet(HELPER_ICON_STYLE)
        self.interval_lock_icon.hide()
        interval_layout.addWidget(self.interval_lock_icon)
        ...
        self.multiplier_info = self._create_helper_icon('Steering duration multiplier. 1.0 is tuned as a good Seawise baseline.\nVehicles that turn slower than Seawise may need multiplier > 1.\nVehicles that turn faster may need multiplier < 1.')
        multiplier_layout.addWidget(self.multiplier_info)
    ...
    def _build_process_guard_section(self, layout):
        ...
        self.guard_fold_btn.setToolTip('Show or hide settings to stop Windows services and end processes each cycle.')
        ...
        services_label.setToolTip('Optional comma-separated service short names (SCM names) to stop each cycle when running - e.g. WpcMonSvc. Not the Task Manager display name.')
        ...
        process_label.setToolTip('Optional comma-separated image names to close each cycle - e.g. WpcMon.exe, msedge.exe (same idea as AeroMulti process field).')
    ...
    def _build_issues_section(self, layout):
        ...
        fusion = QStyleFactory.create('Fusion')
        if fusion:
            self.issues_view.setStyle(fusion)
        self.issues_view.setReadOnly(True)
        self.issues_view.setFrameShape(QFrame.NoFrame)
        self.issues_view.setLineWrapMode(QTextEdit.WidgetWidth)
        ...
        self.issues_view.setFont(QFont(APP_FONT_FAMILY, 9))
        self.issues_view.setMinimumHeight(42)
        self.issues_view.setMaximumHeight(120)
        self.issues_view.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        self.issues_view.document().setDocumentMargin(6)
        self.issues_view.setFocusPolicy(Qt.NoFocus)
        self.issues_view.setHtml(f"<div style='font-family:{APP_FONT_FAMILY};font-size:9px;color:{TEXT_MUTED};'>Loading issues...</div>")
        self.issues_view.setStyleSheet(ISSUES_VIEW_STYLE)
    ...
    def update_issues(self, issues):
        priority_colors = {'high': ERROR_TEXT, 'medium': '#F9B84B', 'low': WARNING_TEXT}
        priority_labels = {'high': 'HIGH', 'medium': 'MED', 'low': 'LOW'}
        if not issues:
            self.issues_view.setHtml(f"<div style='font-family:{APP_FONT_FAMILY};font-size:9px;color:{TEXT_MUTED};padding:2px;'>No active issues.</div>")
            self.issues_view.setStyleSheet(ISSUES_VIEW_STYLE)
            self.issues_view.verticalScrollBar().setStyleSheet(_STATUS_VIEW_SCROLLBAR_QSS)
            return
        parts = []
        for i, issue in enumerate(issues):
            ...
            color = priority_colors.get(priority, WARNING_TEXT)
            badge = priority_labels.get(priority, priority.upper())
            ...
            if i > 0:
                parts.append(f"<div style='height:5px;'></div>")
            bar_bg = 'rgba(255,255,255,0.10)'
            bar_fill = color
            bar_html = f"<div style='margin-top:5px;background:{bar_bg};border-radius:3px;height:4px;width:100%;max-width:280px;'><div style='background:{bar_fill};border-radius:3px;height:4px;width:{progress}%;'></div></div><span style='color:{TEXT_SUBTLE};font-size:8px;'>{progress}%</span>"
            parts.append(f"<div style='font-family:{APP_FONT_FAMILY};padding:6px 8px;border:1px solid rgba(255,255,255,0.10);border-left:3px solid {color};background:rgba(255,255,255,0.045);border-radius:10px;'><span style='font-size:8px;font-weight:700;color:{color};letter-spacing:0.45px;'>{badge}</span>&nbsp;<span style='font-size:10px;font-weight:700;color:{TEXT_PRIMARY};'>{title}</span><br>" + (f"<span style='font-size:9px;color:{TEXT_MUTED};line-height:1.25;'>{desc}</span><br>" if desc else '') + bar_html + f'</div>')
        self.issues_view.setHtml(''.join(parts))
        self.issues_view.setStyleSheet(ISSUES_VIEW_STYLE)
        self.issues_view.verticalScrollBar().setStyleSheet(_STATUS_VIEW_SCROLLBAR_QSS)
    ...
    def set_global_version(self, global_version):
        ...
        if is_version_outdated(self.version, global_version):
            ...
            self.version_check_label.setStyleSheet(PILL_STYLE + f'color: {ERROR_TEXT};')
            ...
            self.version_check_label.setStyleSheet(PILL_STYLE + f'color: {SUCCESS_TEXT};')
    ...
    def _checkbox_definitions(self):
        return ((self.mid_mission_checkbox, self.config.get_start_mid_mission()), (self.custom_waypoint_checkbox, self._custom_waypoint_enabled_for_mode()), (self.skip_bearing_checkbox, self.config.get_monitoring_skip_current_bearing()), (self.throttle_up_checkbox, self.config.get_throttle_up_if_not_100()), (self.quit_after_5_errors_checkbox, self.config.get_quit_after_5_errors()), (self.include_screenshots_checkbox, self.config.get_include_screenshots()), (self.share_data_checkbox, self.config.get_share_data_with_developer()))

    def _checkbox_label(self, checkbox):
        for widget, label in ((self.mid_mission_checkbox, 'Start Mid-Mission'), (self.custom_waypoint_checkbox, 'Custom waypoint'), (self.skip_bearing_checkbox, 'Unlock 5 view'), (self.throttle_up_checkbox, 'Throttle up if not 100%'), (self.quit_after_5_errors_checkbox, 'Quit after 5 consecutive errors (recommended)'), (self.include_screenshots_checkbox, 'Include screenshots in Mission Status'), (self.share_data_checkbox, 'Share data with developer')):
            if checkbox == widget:
                return label
        return None
    ...
    def _set_controls_enabled(self, enabled):
        ...
        self.multiplier_input.setEnabled(enabled)
    ...
    def _set_initial_window_size(self):
        ...
        if screen:
            ...
            width = min(max(int(available.width() * 0.22), 380), 560)
    ...
    def _update_layout_for_window_size(self):
        ...
        if maximized or h >= 900:
            ...
            self.issues_view.setMinimumHeight(max(90, int(h * 0.12)))
        elif h >= 720:
            self.status_view.setMaximumHeight(max(170, int(h * 0.22)))
            self.issues_view.setMaximumHeight(max(120, int(h * 0.16)))
            self.status_view.setMinimumHeight(90)
            self.issues_view.setMinimumHeight(60)
        else:
            self.status_view.setMaximumHeight(170)
            self.issues_view.setMaximumHeight(120)
            self.status_view.setMinimumHeight(74)
    ...
    def _apply_theme(self):
        ...
        if app is not None:
            ...
            if 'QToolTip {' not in existing:
                app.setStyleSheet(existing + TOOLTIP_STYLE)
    ...
    def load_config(self):
        ...
        if notification_mode == 'urgent':
            self.notification_mode_combo.setCurrentIndex(1)
        else:
            self.notification_mode_combo.setCurrentIndex(0)
    ...
    def unlock_fields(self):
        ...
        self.interval_lock_icon.setVisible(is_autopilot)
    ...
    def on_mode_changed(self, text):
        ...
        self.notification_mode_label.setVisible(is_monitoring)
        self.notification_mode_combo.setVisible(is_monitoring)
        self.custom_waypoint_checkbox.setVisible(is_monitoring or is_autosteer)
        self.custom_waypoint_info.setVisible(is_monitoring or is_autosteer)
        self.skip_bearing_checkbox.setVisible(is_monitoring)
        self.skip_bearing_info.setVisible(is_monitoring)
        self.throttle_up_checkbox.setVisible(is_monitoring or is_autosteer)
        self.throttle_up_info.setVisible(is_monitoring or is_autosteer)
    ...
    def on_notification_mode_changed(self, text):
        mode = 'all' if text == 'All Notifications' else 'urgent'
