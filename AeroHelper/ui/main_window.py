...
THROTTLE_UP_TOOLTIP = 'When checked: if OCR reads throttle below 100%, hold W for 10 seconds to restore full throttle.\n\nApplies in Monitoring, AutoSteer, and AutoPilot cruise.\n\nSkipped during AutoPilot docking/undocking where lower throttle is intentional.\n\nWhen off: no throttle-up action is taken.'
QUIT_AFTER_5_ERRORS_TOOLTIP = 'Highly recommended.'
INTELLIGENT_STEERING_TOOLTIP = 'Beta feature: starts the steering multiplier at 1.0, then slowly hones in on a value that matches how your vehicle actually turns.\n\nIf the ship understeers, the multiplier goes up. If it oversteers, it goes down. After a few corrections it should settle on a multiplier that fits that vehicle.\n\nThe manual multiplier is ignored while this is on. Still in beta — results vary by vehicle.'
...
class MainWindow(QMainWindow):
    ...
    def _build_option_checkboxes(self, layout):
        ...
        intelligent_beta = QLabel('BETA')
        intelligent_beta.setFont(QFont(APP_FONT_FAMILY, scale_font(8), QFont.Bold))
        intelligent_beta.setToolTip(self._format_tooltip(INTELLIGENT_STEERING_TOOLTIP))
        intelligent_beta.setStyleSheet(f'''
            color: {WARNING_TEXT};
    ...
    def _build_cycle_controls(self, layout):
        ...
        self.interval_lock_icon.setToolTip(self._format_tooltip('Locked: AutoPilot uses 15s cycle delay'))
    ...
    def _build_issues_section(self, layout):
        ...
        self.issues_count_label.setFont(QFont(APP_FONT_FAMILY, scale_font(8), QFont.Bold))
        self.issues_count_label.setStyleSheet(PILL_STYLE + f'color: {TEXT_MUTED};')
    ...
    def _create_issue_card(self, issue):
        ...
        badge = QLabel(theme['label'])
        badge.setFont(QFont(APP_FONT_FAMILY, scale_font(8), QFont.Bold))
    ...
    def update_issues(self, issues):
        ...
        if not issues:
            ...
            self.issues_count_label.setStyleSheet(PILL_STYLE + f'color: {SUCCESS_TEXT};')
        ...
        self.issues_count_label.setStyleSheet(PILL_STYLE + f'color: {theme["text"]};')
    ...
    def on_start_clicked(self):
        ...
        try:
            ...
            return
        intelligent_on = self.config.get_mode() in ('AutoSteer', 'AutoPilot') and self.intelligent_steering_checkbox.isChecked()
        if not intelligent_on:
            try:
    ...
    def on_mode_changed(self, text):
        ...
        self.throttle_up_checkbox.setVisible(is_monitoring or is_autosteer or is_autopilot)
        ...
        if is_monitoring or is_autosteer or is_autopilot:
