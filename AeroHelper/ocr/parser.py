...
class OCRParser:
    ...
    def __init__(self):
        ...
        self.patterns = {
            ...
            # % is optional: EasyOCR often emits "Fuel: 83.29 Abandon" / "Throttle: 100" with no percent sign
            'fuel': re.compile(rf'Fuel{sep}(\d+(?:\.\d+)?)\s*%?', re.IGNORECASE),
            'distance': re.compile(rf'Distance{sep}(\d+(?:\.\d+)?)\s*({dist})', re.IGNORECASE),
            'heading': re.compile(
                r'(?:HDG|HOG|H0G|TRK)[:.\s]*(\d{1,3})(?!\.\d)',
                re.IGNORECASE,
            ),
        }
    ...
    def _normalize_ocr_text(self, text):
        ...
        # OCR letter O/o misread as zero near HUD labels (e.g. "Throttle: O %")
        normalized = re.sub(
            r'\b(Speed|Thrott?le|Altitude|Fuel)([.:\s]+)[Oo](?=\s*%|\s*(?:Knots?|knot[5s]?|ft)\b)',
            r'\1\g<2>0',
            normalized,
            flags=re.IGNORECASE,
        )
        normalized = re.sub(r'\bH[\s]?0[\s]?G\b', 'HDG', normalized, flags=re.IGNORECASE)
        normalized = re.sub(r'\bHD[\s]*G\b', 'HDG', normalized, flags=re.IGNORECASE)
        return normalized

    def _looks_like_dropped_leading_digit(self, lo, hi):
        if hi <= lo:
            return False
        # Classic: 38.72 vs 138.72 (hi ≈ lo + 100)
        if abs((hi - lo) - 100.0) <= 2.0:
            return True
        lo_s = f'{lo:.4f}'.rstrip('0').rstrip('.')
        hi_s = f'{hi:.4f}'.rstrip('0').rstrip('.')
        if hi_s.endswith(lo_s) and len(hi_s) == len(lo_s) + 1 and hi_s[0] in '123456789':
            return True
        return False

    def _pick_distance(self, vals, previous_distance=None):
        if not vals:
            return None
        if len(vals) == 1:
            return vals[0]
        ...
        if hi - lo <= 2.0:
            return lo
        if self._looks_like_dropped_leading_digit(lo, hi):
            return hi
        if previous_distance is not None:
            return min(vals, key=lambda v: abs(v - previous_distance))
        # Disagreeing engines with no prior: prefer larger (WinRT often keeps the leading digit)
        return hi
    ...
    def parse(self, text, preferred_code=None, prefer_custom=False, previous_distance=None):
        ...
        if distance_matches:
            try:
                ...
                data.distance = round_distance_nm(self._pick_distance(vals, previous_distance=previous_distance))
            except (ValueError, IndexError, TypeError):
                pass
        heading_matches = list(self.patterns['heading'].finditer(normalized_text))
        if heading_matches:
            try:
                # Prefer an HDG/HOG match over TRK (TRK is ground track fallback)
                chosen = None
                for m in heading_matches:
                    label = m.group(0)[:3].upper()
                    if label.startswith('HD') or label.startswith('HO') or label.startswith('H0'):
                        chosen = m
                if chosen is None:
                    chosen = heading_matches[-1]
                value = int(chosen.group(1))
                if 0 <= value <= 360:
                    data.heading = value
            except (ValueError, IndexError):
                pass
