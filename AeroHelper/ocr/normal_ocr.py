...
class NormalOCR:
    ...
    def _run_windows_coro(self, coro):
        loop = self._windows_loop
        if loop is None or loop.is_closed():
            loop = asyncio.new_event_loop()
            self._windows_loop = loop
    ...
    def _windows_ocr_extract(self, image_path):
        ...
        try:
            ...
            return self._run_windows_coro(self._windows_ocr_extract_async(image_path))
        except Exception as e:
            import traceback
    ...
    def _windows_ocr_word_boxes(self, image_path):
        if not _WINSDK_AVAILABLE:
            return []
        try:
            words = self._run_windows_coro(self._windows_ocr_words_async(image_path))
            return words
        except Exception as e:
            import traceback
    ...
    def _native_has_hud_basics(self, text):
        if not text or len(text) < 20:
            return False
        if self._hud_parser is None:
            from AeroHelper.ocr.parser import OCRParser
            self._hud_parser = OCRParser()
        data = self._hud_parser.parse(text)
        return data.distance is not None or data.speed is not None
    ...
    def extract_text(self, image_path, return_boxes=False):
        ...
        skip_easyocr = (not return_boxes) and self._native_has_hud_basics(native_text)
        if skip_easyocr:
            easyocr_boxes, easyocr_text = (None, '')
            if not self.ocr_debug_mode:
                self._log('info', 'OCR: Native HUD text present - skipping EasyOCR')
        else:
