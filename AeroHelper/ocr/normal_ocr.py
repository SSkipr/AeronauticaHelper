...
class NormalOCR:
    ...
    def __init__(self, logger=None, ocr_debug_mode=False):
        self.logger = logger
        self.ocr_debug_mode = bool(ocr_debug_mode)
    ...
    def _easyocr_extract(self, image_path, return_boxes=False):
        ...
        try:
            ...
            if not self.ocr_debug_mode:
                self._log('info', f'EasyOCR: Processing took {elapsed:.2f}s')
            text_parts = []
            ...
            if not self.ocr_debug_mode:
                self._log('info', f'''EasyOCR: Extracted text (length: {len(text)}, {len(results)} regions, {high_confidence_count}/{len(results)} high-confidence, avg confidence: {avg_confidence:.3f})''')
            if return_boxes:
    ...
    def _vision_ocr_extract(self, image_path):
        ...
        try:
            ...
            elapsed = time.perf_counter() - t0
            if not self.ocr_debug_mode:
                self._log('info', f'Vision OCR: Processing took {elapsed:.2f}s, extracted text (length: {len(combined)}, {len(results)} regions): "{combined}"')
            return combined
        except Exception as e:
            import traceback
    ...
    async def _windows_ocr_extract_async(self, image_path):
        ...
        try:
            ...
            try:
                ...
                data_writer.write_bytes(byte_array)
            ...
            elapsed = time.perf_counter() - t0
            if not self.ocr_debug_mode:
                self._log('info', f'Windows OCR: Processing took {elapsed:.2f}s, extracted text (length: {len(combined_text)}, {len(result.lines)} lines, {total_words} words): "{combined_text}"')
    ...
    def _windows_ocr_extract(self, image_path):
        ...
        try:
            ...
            result = loop.run_until_complete(self._windows_ocr_extract_async(image_path))
            loop.close()
            return result
        except Exception as e:
            import traceback
    ...
    async def _windows_ocr_load_bitmap_async(self, image_path):
        try:
            ...
            stream = InMemoryRandomAccessStream()
            data_writer = DataWriter(stream)
            data_writer.write_bytes(list(image_data))
            await data_writer.store_async()
            await data_writer.flush_async()
            stream.seek(0)
            decoder = await BitmapDecoder.create_async(stream)
    ...
    def _windows_ocr_word_boxes(self, image_path):
        ...
        try:
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            words = loop.run_until_complete(self._windows_ocr_words_async(image_path))
            loop.close()
            return words
        except Exception as e:
            import traceback
    ...
    def extract_text(self, image_path, return_boxes=False):
        if not self.ocr_debug_mode:
            self._log('info', f'OCR: Starting extraction from {image_path}')
        ...
        if not self.ocr_debug_mode:
            self._log('info', f'OCR: EasyOCR text: "{easyocr_text}"')
            if _WINSDK_AVAILABLE:
                self._log('info', f'OCR: WinRT text: "{native_text}"')
            elif _VISION_AVAILABLE:
                self._log('info', f'OCR: Vision text: "{native_text}"')
        ...
        if self.ocr_debug_mode:
            ...
            if return_boxes:
                context = f'extract_text boxes={len(easyocr_boxes or [])}'
            self._log_ocr_bundle(context, image_path, sections)
        if return_boxes:
    ...
    def find_text_boxes(self, image_path, target_text, match_mode='contains', prefer_windows=False, allow_easyocr=True):
        """Find text boxes. prefer_windows=True tries WinRT (~0.2s) before EasyOCR (~20s).
        Set allow_easyocr=False to skip EasyOCR (use for time-critical flyout OCR)."""
    ...
    def create_left_nav_roi(self, image_path, play_center=None, width_frac=0.32, pad_y=200):
        """Crop left-nav / Play-flyout region. Returns (roi_path, (offset_x, offset_y)) or (None, None)."""
        try:
            img = Image.open(image_path).convert('RGB')
            w, h = img.size
            if play_center and len(play_center) >= 2:
                cx, cy = int(play_center[0]), int(play_center[1])
                left = max(0, cx - 80)
                right = min(w, max(cx + int(w * width_frac), left + 220))
                top = max(0, cy - int(pad_y * 0.45))
                bottom = min(h, cy + pad_y)
            else:
                left, top = 0, int(h * 0.2)
                right, bottom = int(w * width_frac), int(h * 0.9)
            ...
            self._log('warning', f'Left-nav ROI crop failed: {e}')
            return (None, None)
    ...
    def analyze_text_color(self, image_path, box_coords):
        try:
            import numpy as np
            img = Image.open(image_path).convert('RGB')
            ...
            max_x = min(img_array.shape[1], int(max(x_coords)))
            min_y = max(0, int(min(y_coords)))
            max_y = min(img_array.shape[0], int(max(y_coords)))
            ...
            dist_red = ((r - 255) ** 2 + g ** 2 + b ** 2) ** 0.5
            self._log('info', f'analyze_text_color: avg=({r:.0f},{g:.0f},{b:.0f}), dist_white={dist_white:.1f}, dist_red={dist_red:.1f}')
            if dist_white < dist_red:
                return 'white'
            else:
                return 'red'
        except Exception as e:
            self._log('error', f'analyze_text_color failed: {str(e)}')
            return 'unknown'
