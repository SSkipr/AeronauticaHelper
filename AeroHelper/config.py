...
class Config:
    ...
    def get_notification_mode(self):
        from AeroHelper.notifications.policy import normalize_notification_mode
        return normalize_notification_mode(self.get('NOTIFICATION_MODE', 'minimal'))

    def set_notification_mode(self, mode):
        from AeroHelper.notifications.policy import normalize_notification_mode
        self.set('NOTIFICATION_MODE', normalize_notification_mode(mode))

    def get_notification_custom_pings(self):
        from AeroHelper.notifications.policy import normalize_custom_pings
        raw = self.get('NOTIFICATION_CUSTOM_PINGS', '')
        if not raw:
            return normalize_custom_pings(None)
        try:
            parsed = json.loads(raw)
        except (TypeError, ValueError, json.JSONDecodeError):
            parsed = None
        return normalize_custom_pings(parsed)

    def set_notification_custom_pings(self, pings):
        from AeroHelper.notifications.policy import normalize_custom_pings
        self.set('NOTIFICATION_CUSTOM_PINGS', json.dumps(normalize_custom_pings(pings)))

    def build_notification_policy(self):
        from AeroHelper.notifications.policy import NotificationPolicy
        return NotificationPolicy(self.get_notification_mode(), self.get_notification_custom_pings())
    ...
    def get_intelligent_steering(self):
        return self.get('INTELLIGENT_STEERING', 'false').lower() == 'true'

    def set_intelligent_steering(self, enabled):
        self.set('INTELLIGENT_STEERING', str(enabled).lower())
    ...
    def get_ocr_debug(self):
        return self.get('OCR_DEBUG', 'false').lower() == 'true'

    def set_ocr_debug(self, enabled):
        self.set('OCR_DEBUG', str(enabled).lower())
    ...
    def history_path(self):
        return self.config_dir / 'AeroHelper.history.json'

    def _load_history_from_env(self):
    ...
    def get_history(self):
        ...
        if path.exists():
            try:
                ...
                entries = None
        if entries is None:
            entries = self._load_history_from_env()
            if entries:
                self.set_history(entries)
                try:
                    self.set('AEROHELPER_HISTORY', '')
                except Exception:
                    pass
        trimmed = self._trim_history(entries or [])
        if trimmed != entries:
