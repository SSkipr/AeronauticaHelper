...
from AeroHelper.utils.halt import raise_if_halted
    ...
    def click(self, x, y, smooth=True):
        ...
        try:
            ...
            if not smooth:
                if self.using_mousekey:
                    try:
                        click_xy = getattr(self.mouse, 'left_click_xy', None)
                        if callable(click_xy):
                            click_xy(x, y)
                        else:
                            self.mouse.move_to(x, y)
                            time.sleep(0.05)
                            left_click = getattr(self.mouse, 'left_click', None)
                            if callable(left_click):
                                left_click()
                            else:
                                self.controller.click(Button.left, 1)
                        self._log('debug', f'Clicked at ({x}, {y}) using MouseKey (instant)')
                        return
                    except Exception as e:
                        self._log('debug', f'MouseKey instant click failed ({e}); using pynput')
