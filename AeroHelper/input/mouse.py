...
from AeroHelper.utils.window import get_roblox_window_center
...
class Mouse:
    ...
    def _move_to_pynput(self, x, y):
        self.controller.position = (int(x), int(y))

    def move_to(self, x, y, natural=True):
        """Move cursor. Prefer MouseKey so Roblox receives real input events (needed for Play hover)."""
        try:
            x, y = (int(x), int(y))
            if self.using_mousekey:
                try:
                    if natural:
                        self.mouse.move_to_natural(
                            x,
                            y,
                            min_variation=-2,
                            max_variation=2,
                    ...
                    self._log('debug', f'MouseKey move failed ({e}); using pynput')
            self._move_to_pynput(x, y)
        except Exception as e:
    ...
    def _click_pynput(self, x, y):
        ...
        self._move_to_pynput(final_x, final_y)
        time.sleep(0.05)
    ...
    def click(self, x, y, smooth=True):
        try:
            ...
            if x < 0 or y < 0:
                ...
                return
            if not smooth:
                self.move_to(x, y, natural=False)
                time.sleep(0.05)
                self.controller.click(Button.left, 1)
                backend = 'MouseKey+pynput' if self.using_mousekey else 'pynput'
                self._log('debug', f'Clicked at ({x}, {y}) using {backend} (instant)')
                return
            if self.using_mousekey:
                try:
    ...
    def hover(self, x, y, dwell=0.55):
        """Move cursor onto a target without clicking (keeps Play flyouts open). Uses MouseKey when available."""
        try:
            x, y = (int(x), int(y))
            backend = 'MouseKey' if self.using_mousekey else 'pynput'
            self.move_to(x, y, natural=True)
            time.sleep(0.12)
            offset_x = random.randint(-2, 2)
            offset_y = random.randint(-2, 2)
            self.move_to(x + offset_x, y + offset_y, natural=True)
            time.sleep(max(0.15, float(dwell)))
            self.move_to(x, y, natural=True)
            self._log('debug', f'Hovered at ({x}, {y}) dwell={dwell:.2f}s via {backend} (no click)')
        except Exception as e:
