...
def _is_roblox_process(proc_info):
    proc_name = (proc_info.get('name') or '').lower()
    if proc_name and ('roblox' in proc_name or 'robloxplayerbeta' in proc_name):
        return True
    exe = (proc_info.get('exe') or '').lower()
    return 'roblox' in exe

def is_roblox_running():
    try:
        for proc in psutil.process_iter(['pid', 'name', 'exe']):
            try:
                if _is_roblox_process(proc.info):
