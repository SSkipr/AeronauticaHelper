...
def _append_bearing_match(matches, code, raw_bearing):
...
def _collect_airport_matches(text_upper):
    ...
    for code in ICAO_CODES:
        ...
        for pattern_str in patterns:
            ...
            for match in pattern.finditer(text_upper):
                try:
                    _append_bearing_match(matches, code, match.group(1))
                except ValueError:
                    pass
    return matches
    ...
    for pattern in (_NON_AIRPORT_ALPHA_PATTERN, _NON_AIRPORT_DECIMAL_CODE_PATTERN):
        for match in pattern.finditer(text_upper):
            ...
            if not _is_non_airport_code(code):
                continue
            try:
                _append_bearing_match(matches, code, match.group(2))
            except ValueError:
                pass
    return matches
...
def extract_icao_and_bearing(text):
    text_upper = _normalize_text(text)
    ...
    if airport_matches:
        if _dest_fragment_visible(text_upper):
            ...
            if ref is not None:
                return ('DEST', ref)
        return (airport_matches[-1][0], airport_matches[-1][1])
    non_airport_matches = _collect_non_airport_matches(text_upper)
    if non_airport_matches:
        return (non_airport_matches[-1][0], non_airport_matches[-1][1])
    if _is_final_approach_context(text_upper):
        ref = _heading_ref_from_text(text_upper)
        if ref is not None:
            return ('DEST', ref)
    return (None, None)
