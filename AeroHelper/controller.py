...
class Controller:
    ...
    def __init__(self, main_window, status_overlay, app_version=None):
    ...
    def _create_automation_mode(self):
        ...
        if mode_str == 'Monitoring':
            ...
            return (MonitoringMode(cycle_interval, webhook_url, self.logger, self.human_detector, notification_mode, custom_waypoint=custom_waypoint, include_screenshots=include_screenshots, keyboard=shared_keyboard), mode_str, cycle_interval, multiplier, notification_mode, webhook_url, include_screenshots, custom_waypoint)
        if mode_str == 'AutoSteer':
            custom_waypoint = self.config.get_autosteer_custom_waypoint()
            return (AutoSteerMode(cycle_interval, webhook_url, multiplier, self.logger, self.human_detector, custom_waypoint=custom_waypoint, include_screenshots=include_screenshots, keyboard=shared_keyboard), mode_str, cycle_interval, multiplier, notification_mode, webhook_url, include_screenshots, custom_waypoint)
        if mode_str == 'AutoPilot':
            start_mid_mission = self.config.get_start_mid_mission()
            return (AutoPilotMode(cycle_interval, webhook_url, multiplier, self.logger, self.human_detector, start_mid_mission=start_mid_mission, include_screenshots=include_screenshots, history_callback=self.add_history_entry, keyboard=shared_keyboard), mode_str, cycle_interval, multiplier, notification_mode, webhook_url, include_screenshots, None)
    ...
    def _start_automation_worker_impl(self):
        ...
        def delayed_start():
            ...
            if IS_MACOS and not self._roblox_focus_ok:
                countdown_secs = 8
                self.logger.info(f'[STARTUP] Roblox focus failed on macOS - extending countdown to {countdown_secs}s (click Roblox now)')
            self._sleep_with_running_check(countdown_secs)
    ...
    def stop(self, clear_error=True):
        try:
            ...
            try:
                ...
                if clear_error:
                    QMetaObject.invokeMethod(self.main_window, 'clear_error', Qt.QueuedConnection)
            except Exception as e:
            ...
            try:
                ...
                if action == 'pause':
                    ...
                    if reason == 'Invalid game state' and self._startup_grace_remaining > 0:
                        ...
                        self.logger.warning(f'[STARTUP GRACE] Invalid game state - retrying in 5s ({remaining} retr{"y" if remaining == 1 else "ies"} left). Ensure Roblox is visible and mid-mission.')
                        self._emit_main_status('warning', f'Waiting for game state... ({remaining} retr{"y" if remaining == 1 else "ies"} left)\n\nMake sure Roblox is visible and you are mid-mission.', self._build_diagnostics())
    ...
    def _handle_pause_with_error(self, reason, details=''):
        ...
        if reason == 'Invalid game state':
            ...
            error_message = 'AeroHelper could not detect valid mission data from the screen.\n\nThis usually means:\n• You are not mid-mission in Aeronautica\n• The game UI is not visible or obscured\n• OCR failed to read the screen properly\n• The game window is minimized or covered\n\nPlease ensure:\n1. Roblox/Aeronautica is running and visible\n2. You are mid-mission (not in lobby or menu)\n3. The game window is not minimized or covered\n4. Display mode is set to DUPLICATED (not EXTEND)\n5. The game UI showing mission data is visible'
        elif reason == 'Return to Lobby detected':
            error_title = 'Return to Lobby Detected'
            error_message = "AeroHelper detected the 'Return to Lobby' button on screen.\n\nAeroHelper can only run mid-mission. Please:\n1. Start a mission in Aeronautica\n2. Wait until you are in an active mission\n3. Then resume AeroHelper"
        elif reason == 'Distance threshold reached':
            error_title = 'Destination Approaching'
            error_message = 'AeroHelper has stopped near the destination.\n\nThe remaining distance has reached the safety threshold.\nPlease manually dock the vehicle.\nYou can resume AeroHelper after docking or starting a new mission.'
        elif reason == 'Vehicle going away - stopped after 10 cycles':
            error_title = 'Vehicle Going Away'
            error_message = 'AeroHelper detected the vehicle moving away from the destination for 10 consecutive cycles.\n\nThe vehicle has been stopped (Z pressed).\n\nThis usually means:\n• The vehicle is heading in the wrong direction\n• Steering corrections may have failed\n• Check your heading and destination manually\n\nPlease correct the heading and resume AeroHelper.'
            ...
            error_message = 'Docking did not complete after the full retry sequence. Manual docking is required.\n\nAeroHelper stopped after two final-dock rounds (including long Z hold).\nPlease dock manually, then start a new mission or restart AutoPilot from the app.'
            ...
            error_message = f"AeroHelper detected a waypoint ({waypoint_code}) instead of the final destination.\n\nAeroHelper can only operate when heading directly to the final destination (DEST).\n\nWhen navigating to a waypoint:\n• The distance shown is to the final destination\n• The heading is toward the waypoint\n• This creates incorrect distance calculations\n\nPlease wait until the waypoint is reached and the ICAO code shows 'DEST',\nthen resume AeroHelper."
        else:
            extra = (details or '').strip()
            if extra:
                error_message = f'Automation has been paused: {reason}\n\n{extra}'
            else:
                error_message = f'Automation has been paused: {reason}\n\nPlease check the game state and restart AeroHelper.'
    ...
    def _handle_5_consecutive_errors(self, last_error):
        self.logger.error('5 consecutive errors - stopping and sending @everyone to Discord')
        ...
        try:
            self.automation_mode.notifier.send_error(f'AeroHelper stopped after 5 consecutive errors. Last error: {last_error}')
        except Exception as e:
            self.logger.error(f'Failed to send 5-errors webhook: {e}')
        self.main_window.show_error_signal.emit('AeroHelper Stopped', f'Stopped after 5 consecutive errors. Last error: {last_error}\n\nCheck Discord for more context. Restart AeroHelper when ready.', '')
        self.main_window.is_running = False
        QMetaObject.invokeMethod(self.main_window, 'show_start_button', Qt.QueuedConnection, Q_ARG(bool, False))
        self.stop(clear_error=False)
