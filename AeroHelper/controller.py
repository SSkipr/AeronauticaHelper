...
class Controller:
    ...
    def _create_automation_mode(self):
        ...
        if mode_str in ('AutoSteer', 'AutoPilot') and intelligent_steering:
            multiplier = 1.0
        ...
        if mode_str == 'AutoSteer':
            ...
            return (AutoSteerMode(cycle_interval, webhook_url, multiplier, self.logger, self.human_detector, custom_waypoint=custom_waypoint, include_screenshots=include_screenshots, keyboard=shared_keyboard, throttle_up_if_not_100=throttle_up, intelligent_steering=intelligent_steering), mode_str, cycle_interval, multiplier, notification_mode, webhook_url, include_screenshots, custom_waypoint)
        if mode_str == 'AutoPilot':
            start_mid_mission = self.config.get_start_mid_mission()
            return (AutoPilotMode(cycle_interval, webhook_url, multiplier, self.logger, self.human_detector, start_mid_mission=start_mid_mission, include_screenshots=include_screenshots, history_callback=self.add_history_entry, keyboard=shared_keyboard, throttle_up_if_not_100=throttle_up, intelligent_steering=intelligent_steering), mode_str, cycle_interval, multiplier, notification_mode, webhook_url, include_screenshots, None)
    ...
    def _log_session_start(self, mode_str, cycle_interval, multiplier, notification_mode, webhook_url, include_screenshots, custom_waypoint=None):
        ...
        if mode_str in ('AutoSteer', 'AutoPilot'):
            ...
            lines.insert(5, f'[SESSION] Intelligent steering: {intelligent}' + (' (beta; starts at 1.0 and hones in)' if intelligent else ''))
            ...
            try:
                ...
                if action == 'pause':
                    ...
                    if reason == 'Distance threshold reached':
                        self.logger.info(f'Cycle pause: {reason} (destination reached)')
    ...
    def _handle_destination_reached(self, result=None):
        ...
        try:
            ...
            self.logger.error(f'Failed to send destination-reached webhook: {e}')
