...
# Jobs flyout sits to the right of Play in the left nav (screenshot/screen px).
...
class AutoPilotMode(AutoSteerMode):
    ...
    def __init__(self, cycle_interval, webhook_url, multiplier, logger, human_detector=None, start_mid_mission=False, include_screenshots=False, history_callback=None, keyboard=None, throttle_up_if_not_100=False):
        super().__init__(cycle_interval, webhook_url, multiplier, logger, human_detector, include_screenshots=include_screenshots, keyboard=keyboard, throttle_up_if_not_100=throttle_up_if_not_100)
    ...
    def _get_cached_ui_coords(self, label):
        ...
        try:
            ...
            coords = Config().get_ui_button_coords(label)
            if coords:
    ...
    def _save_cached_ui_coords(self, label, x, y):
        ...
        try:
            from AeroHelper.config import Config
            Config().set_ui_button_coords(label, coords[0], coords[1])
        except Exception as e:
    ...
    def _clear_cached_ui_coords(self, *labels):
        for label in labels:
            ...
            try:
                from AeroHelper.config import Config
                Config().clear_ui_button_coords(label)
            except Exception:
                pass

    def _locate_ui_label(self, target_text, match_mode='word', prefer_windows=True):
        """Return (screen_x, screen_y, screenshot_center) or None. Prefer WinRT for Play/Jobs speed."""
        screenshot_path = None
        try:
            screenshot_path = capture_primary_screen()
            if not screenshot_path or not os.path.exists(screenshot_path):
    ...
    def _pick_jobs_match(self, matches):
        """Prefer Jobs near left-nav flyout (AeroMulti sort)."""
        if not matches:
            return None
    ...
    def _snap_jobs_coords_fast(self):
        """Screenshot while flyout is open; WinRT-only so we can click before it closes."""
        screenshot_path = None
        try:
            screenshot_path = capture_primary_screen()
            if not screenshot_path or not os.path.exists(screenshot_path):
            ...
            matches = self._filter_ui_button_matches(matches, 'Jobs', 'word')
            if not matches:
                matches = self.ocr.find_text_boxes(
                    screenshot_path, 'Jobs', match_mode='contains', prefer_windows=True, allow_easyocr=False
                )
                matches = self._filter_ui_button_matches(matches, 'Jobs', 'contains')
    ...
    def _search_field_visible(self):
        screenshot_path = None
        try:
            screenshot_path = capture_primary_screen()
            if not screenshot_path:
    ...
    def _open_jobs_from_play_menu(self, play_xy):
        """AeroMulti sequence: hover Play → click Play → snap Jobs → double-click Jobs."""
        ...
        # Park on Play so flyout stays open, then snap immediately (WinRT only).
        ...
        # Click Jobs immediately from snap coords (do not OCR again).
        self._click_jobs_at(jobs_xy, double=True, persist=True)
        return True

    def _try_cached_play_to_jobs(self):
        """AeroMulti cached path: open Play, then click Jobs with zero OCR gap."""
    ...
    def _vehicle_preparation(self):
        ...
        for attempt in range(2):
            ...
            if not success:
                # OCR often drops the leading T (urnaround / iurnaround)
                self.logger.info("[AUTOPILOT] Exact 'Turnaround' miss - trying fuzzy '*urnaround*'")
                success, _ = self._find_and_click_text('urnaround', match_mode='contains')
            if success:
    ...
    def _job_search(self):
        ...
        for attempt in range(1, 4):
            ...
            located = self._locate_ui_label('Play', match_mode='word', prefer_windows=True)
            if not located:
                self.logger.warning(
            ...
            # Poll Search like AeroMulti (Jobs panel open confirmation)
            for _ in range(8):
                if not self.running:
                    return False
                if self._search_field_visible():
        ...
        if not self.search_coordinates:
            # One more short wait then retry Search OCR
            self._sleep_if_running(2)
            self.search_coordinates = self._store_click_coordinates('Search', match_mode='exact')
        if not self.search_coordinates:
        ...
        if not success:
            # Coords click already done above; treat as OK if Search was visible
