...
from AeroHelper.utils.bearing import HeadingEWMA, DistanceEWMA
...
class AutoPilotMode(AutoSteerMode):
    ...
    def _jobs_panel_ready(self):
        if self._search_field_visible():
            return True
        return self._get_cached_ui_coords('Search') is not None
    ...
    def _find_and_click_text(self, target_text, instance=1, match_mode='contains', screenshot_path=None, smooth=True):
    ...
    def _hover_text(self, target_text, instance=1, match_mode='contains', click=False, dwell=0.55, smooth=True):
    ...
    def _vehicle_preparation(self):
        ...
        self._hover_text('Your Current Vehicle', click=True, smooth=False)
        time.sleep(1)
        ...
        for attempt in range(2):
            ...
            success, _ = self._find_and_click_text('Turnaround', smooth=False)
            if not success:
                ...
                success, _ = self._find_and_click_text('urnaround', match_mode='contains', smooth=False)
            if success:
        ...
        success = False
        for yes_attempt in range(2):
            if yes_attempt:
                time.sleep(1)
            success, _ = self._find_and_click_text('Yes', match_mode='word', smooth=False)
            if not success:
                success, _ = self._find_and_click_text('Yes', match_mode='exact', smooth=False)
            if success:
                break
        if not success:
