...
class AutoPilotMode(AutoSteerMode):
    ...
    def _job_search(self):
        ...
        for attempt in range(1, 4):
            ...
            if not hovered:
                self.logger.warning(f"[AUTOPILOT] Play not found for hover (attempt {attempt}/3) - close Discord overlay and any modals, then retry")
                time.sleep(0.5)
                continue
            # Re-hover then immediately find Jobs (no webhook gap - flyout closes otherwise)
            time.sleep(0.15)
            self.logger.info(f"[AUTOPILOT] Clicking 'Jobs' immediately after Play hover (attempt {attempt}/3)")
            success, _ = self._find_and_click_text('Jobs', match_mode='word')
            if success:
                jobs_clicked = True
                break
            self.logger.warning(f"[AUTOPILOT] Jobs not found after Play hover (attempt {attempt}/3) - re-hovering Play immediately")
            time.sleep(0.25)
        if not jobs_clicked:
            self.logger.error("[AUTOPILOT] 'Jobs' button not found - keep Roblox focused, close Discord overlay / Special Naming modals, ensure lobby Play flyout can open")
            self._notify_autopilot('Phase 1', 'Jobs button not found', details='Play hover OK but Jobs flyout missing; close overlays and retry')
            return False
        ...
        if not self.search_coordinates:
            self.logger.error('[AUTOPILOT] Could not find Search button to store coordinates - ensure Jobs list is open and visible (no Discord overlay)')
            self._notify_autopilot('Phase 1', 'Search field coordinates failed', details='Jobs opened but Search not visible')
            return False
        ...
        if not success:
            self.logger.error("[AUTOPILOT] 'Search' button not found - Jobs UI may not have loaded; close overlays and retry")
            self._notify_autopilot('Phase 1', 'Search click failed', details='Search label not found after Jobs open')
            return False
