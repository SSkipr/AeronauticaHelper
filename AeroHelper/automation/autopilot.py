...
class AutoPilotMode(AutoSteerMode):
    ...
    def _stuck_distance_recovery_active(self):
        return self.autopilot_phase not in (self.PHASE_1, self.PHASE_2)
        ...
        try:
            if own_screenshot:
                ...
                if not screenshot_path or not os.path.exists(screenshot_path):
                    return (False, None)
            matches = self.ocr.find_text_boxes(screenshot_path, target_text, match_mode=match_mode)
            if matches and match_mode in ('contains', 'word') and target_text.strip().lower() in ('back', 'yes', 'begin', 'play', 'jobs'):
                target_lc = target_text.strip().lower()
                preferred = [m for m in matches if m.get('text', '').strip().lower() == target_lc]
                if not preferred and target_lc == 'jobs':
                    preferred = [m for m in matches if m.get('text', '').strip().lower().startswith('jobs')]
                if preferred:
                    matches = preferred
            if not matches or len(matches) < instance:
                self.logger.warning(f"[AUTOPILOT] find_and_click: '{target_text}' instance {instance} not found (found {len(matches)} matches)")
    ...
    def _hover_text(self, target_text, instance=1, match_mode='contains'):
        screenshot_path = None
        try:
            screenshot_path = capture_primary_screen()
            if not screenshot_path or not os.path.exists(screenshot_path):
            ...
            matches = self.ocr.find_text_boxes(screenshot_path, target_text, match_mode=match_mode)
            if not matches or len(matches) < instance:
                self.logger.warning(f"[AUTOPILOT] hover_text: '{target_text}' instance {instance} not found (found {len(matches)} matches)")
                return False
            target = matches[instance - 1]
            center_x, center_y = target['center']
            click_x, click_y = self._screen_center_to_click_coords(center_x, center_y, screenshot_path)
            self.logger.info(f"[AUTOPILOT] Hovering '{target['text']}' (instance {instance}) at ({click_x}, {click_y})")
            self.mouse.hover(click_x, click_y)
            return True
        except Exception as e:
        ...
        try:
            for spawn_attempt in range(1, PHASE1_SPAWN_MAX_RETRIES + 1):
                ...
                if self.current_airport is None:
                    self.logger.error('[AUTOPILOT] Failed to detect current airport')
                    return self._phase1_give_up('Could not detect current airport', details='Go to a supported airport and resume')
    ...
    def _detect_current_airport(self):
        ...
        try:
            ...
            if not text:
                return None
            currently_at = self._match_currently_at_airport(text)
            if currently_at:
    ...
    def _match_currently_at_airport(self, text):
        if not text:
            return None
        patterns = [re.compile('currently\\s+at\\s+([^\\n\\r,\\.]+?)\\s+airport', re.IGNORECASE), re.compile('you\\s+are\\s+at\\s+([^\\n\\r,\\.]+?)\\s+airport', re.IGNORECASE), re.compile('currently\\s+at\\s+([^\\n\\r,\\.]+)', re.IGNORECASE)]
        candidate = None
        for pat in patterns:
            m = pat.search(text)
            if m:
    ...
    def _vehicle_preparation(self):
        ...
        self._hover_text('Your Current Vehicle')
        time.sleep(1)
    ...
    def _job_search(self):
        self.logger.info("[AUTOPILOT] Hovering 'Play'")
        self._hover_text('Play', match_mode='word')
        time.sleep(1)
        self._notify_autopilot('Phase 1', 'Hovered Play', details='Before Jobs')
        screenshot_path, text, _ = self._take_screenshot_and_ocr()
        if screenshot_path:
            delete_screenshot(screenshot_path)
        self.logger.info("[AUTOPILOT] Clicking 'Jobs'")
    ...
    def _phase1_recover_jobs_ui_after_refresh(self, dest_key):
        ...
        success, _ = self._find_and_click_text('Back')
        if success:
