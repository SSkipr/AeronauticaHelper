...
class MonitoringMode:
    ...
    def _detect_chat_close(self, text):
        # Word-boundary only: "Hidden" (closed panel) must not count as the Hide button
        return bool(re.search(r'\bhide\b', text or '', re.IGNORECASE))
    ...
    def _click_chat_close(self, screenshot_path):
        try:
            ...
            for box_data in boxes:
                ...
                if re.search(r'\bhide\b', box_text):
        ...
        try:
            ...
            if notification_mode in ('urgent-only', 'urgent'):
                if urgent:
                    ...
                    self.notifier.send_urgent_alert(data, self.previous_distance, autosteer_enabled, mode='Monitoring', throttle_up_if_not_100=self.throttle_up_if_not_100, will_throttle_up=will_throttle_up)
                else:
    ...
    def _should_skip_throttle_up(self):
        return False

    def _will_throttle_up(self, data):
        return bool(self.throttle_up_if_not_100 and self._throttle_below_100(data) and (not self._should_skip_throttle_up()))
    ...
    def _is_throttle_urgent(self, data):
        if not self._throttle_below_100(data) or self._should_skip_throttle_up():
            return False
