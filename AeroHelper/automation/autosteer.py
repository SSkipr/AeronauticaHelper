...
from AeroHelper.utils.bearing import shortest_angle_diff_signed, shortest_angle_diff_abs
...
STICKY_BEARING_JUMP_DEG = 40
STICKY_BEARING_CONFIRM_CYCLES = 3
...
class AutoSteerMode(MonitoringMode):
    ...
    def __init__(self, cycle_interval, webhook_url, multiplier, logger, human_detector=None, custom_waypoint=False, include_screenshots=False, keyboard=None, throttle_up_if_not_100=False, intelligent_steering=False):
        ...
        self.multiplier = self._clamp_multiplier(multiplier) if self.intelligent_steering else multiplier
        self.multiplier_persist_callback = None
        ...
        self._sticky_pending_bearing = None
        self._sticky_pending_bearing_cycles = 0
    ...
    def _keep_sticky_on_miss(self):
        if not self._sticky_icao_code or self._sticky_target_bearing is None:
            return None
    ...
    def _sticky_same_code_bearing(self, data):
        new_b = data.target_bearing
        old_b = self._sticky_target_bearing
        self._sticky_pending_code = None
        self._sticky_pending_cycles = 0
        self._sticky_miss_cycles = 0
        if old_b is None or new_b is None:
            self._sticky_pending_bearing = None
            self._sticky_pending_bearing_cycles = 0
            self._sticky_target_bearing = new_b
            return None
        delta = shortest_angle_diff_abs(new_b, old_b)
        if delta <= STICKY_BEARING_JUMP_DEG:
            self._sticky_pending_bearing = None
            self._sticky_pending_bearing_cycles = 0
            self._sticky_target_bearing = new_b
            return None
        if self._sticky_pending_bearing != new_b:
            self._sticky_pending_bearing = new_b
            self._sticky_pending_bearing_cycles = 1
            self.logger.warning(
                f'[AUTOSTEER] DEST jumped {old_b}° -> {new_b}°; waiting '
                f'{STICKY_BEARING_CONFIRM_CYCLES} matching cycles (keeping {old_b}°)'
            )
            return (self._sticky_icao_code, old_b)
        self._sticky_pending_bearing_cycles += 1
        if self._sticky_pending_bearing_cycles >= STICKY_BEARING_CONFIRM_CYCLES:
            self.logger.info(f'[AUTOSTEER] DEST {new_b}° confirmed after jump from {old_b}°')
            self._sticky_target_bearing = new_b
            self._sticky_pending_bearing = None
            self._sticky_pending_bearing_cycles = 0
            return None
    ...
    def _filter_icao_sticky(self, data):
        ...
        if current_code and is_junk_icao_token(current_code):
            ...
            current_code = ''
        if self._sticky_icao_code and is_junk_icao_token(self._sticky_icao_code):
            self._reset_sticky_icao()
        if not current_code:
            return self._keep_sticky_on_miss()
        ...
        if current_code == last_norm:
            return self._sticky_same_code_bearing(data)
    ...
    def _turn_model_degrees_for_hold(self, hold_s):
        wmax = TURN_WMAX_DEG_PER_S
        tramp = TURN_T_RAMP_S
        if hold_s <= 0:
            return 0.0
        if hold_s <= tramp:
            return (hold_s * hold_s) * wmax / tramp
        return hold_s * wmax
    ...
    def _calculate_steering(self, heading, target_bearing, throttle_pct=None):
        ...
        if self._is_dock_steer_phase():
            duration = min(duration, DOCK_TURN_HOLD_MAX_S)
        return (direction, duration, base_h, tscale, turn_deg)

    def _is_dock_steer_phase(self):
        return getattr(self, 'autopilot_phase', None) in ('docking_alignment', 'phase_2')
    ...
    def _set_learned_multiplier(self, new_value, reason):
        ...
        persist = getattr(self, 'multiplier_persist_callback', None)
        if callable(persist):
            try:
                persist(self.multiplier)
            except Exception:
                pass
        return True
    ...
    def _update_intelligent_steering(self, heading):
        ...
        if sample.get('capped') and ratio > 1.0:
            self.logger.debug(
                f'[INTELLIGENT STEERING] Skipping understeer on capped hold '
    ...
    def _record_intelligent_sample(self, heading, direction, turn_deg, duration, oscillated=False, tscale=1.0):
        ...
        if heading is None or not direction or turn_deg is None:
            ...
            return
        if duration is None:
            self.logger.debug('[INTELLIGENT STEERING] Skipping record (hold was missing)')
            self._last_steer_learn = None
            return
        capped = duration >= TURN_HOLD_MAX_S
        expected_deg = float(turn_deg)
        if capped:
            scale = max(1e-6, float(tscale or 1.0) * self.multiplier * self.undocking_steering_multiplier)
            modeled = self._turn_model_degrees_for_hold((duration + TURN_HOLD_TRIM_S) / scale)
            expected_deg = min(expected_deg, modeled)
        ...
        cap_note = ' (capped, oversteer-only)' if capped else ''
        self.logger.debug(
            f'[INTELLIGENT STEERING] Recorded sample: {direction.upper()} expected {expected_deg:.1f}° '
            f'over {duration:.2f}s at multiplier={self.multiplier:.2f}{cap_note}'
        )
    ...
    def _handle_oscillation(self, direction):
        ...
        self.logger.warning(
            f'[AUTOSTEER] Oscillation recovery: keeping direction={direction.upper()}, '
    ...
    def execute_cycle(self):
        ...
        try:
            try:
                ...
                self.logger.warning(f'[AUTOSTEER] Primary screenshot capture failed, trying PIL fallback: {capture_error}')
                try:
                    screenshot_path = capture_screen_pil()
                    self.logger.info('[AUTOSTEER] Successfully used PIL screenshot fallback')
                except Exception as pil_error:
                    import traceback
                    error_msg = f'[AUTOSTEER] Screenshot capture failed completely. Primary error: {capture_error}. PIL error: {pil_error}'
            ...
            if self.ocr.detect_return_to_lobby(text):
                self.logger.warning('[AUTOSTEER] Return to Lobby detected - pausing')
                return {'action': 'pause', 'reason': 'Return to Lobby detected'}
            if self.ocr.detect_disconnected(text):
                self.logger.warning('[AUTOSTEER] Disconnected detected - entering reconnect mode')
            ...
            if not data.valid:
                ...
                self.logger.warning(
                    f'[AUTOSTEER] Invalid game state - distance (nm/km) or speed (knots/km/h) not found. {hint}'
                )
            ...
            if self.requires_dest and self.override_icao_code is None and (data.icao_code is not None) and (data.icao_code.upper() != 'DEST'):
                self.logger.warning(f'[AUTOSTEER] Waypoint detected (ICAO: {data.icao_code}) - AutoPilot requires DEST')
            ...
            if direction and duration is not None:
                ...
                self.logger.info(f'[AUTOSTEER] Steer {direction.upper()} {duration:.2f}s | HDG {data.heading}° → {target_bearing}° (Δ{diff:.1f}°) | steer_HDG={heading_for_steer}° model H₀={model_h0:.2f}s throttle={tp}% hold_scale={tscale:.3f} ω_max={TURN_WMAX_DEG_PER_S} T_ramp={TURN_T_RAMP_S} multiplier={self.multiplier} trim={TURN_HOLD_TRIM_S}s')
                oscillated = False
                if self._detect_oscillation(direction):
                    oscillated = True
                    self._handle_oscillation(direction)
                ...
                self._record_intelligent_sample(heading_for_steer, direction, turn_deg, duration, oscillated=oscillated or stagnated, tscale=tscale)
            else:
                self._reset_heading_stagnation()
            ...
            import traceback
            error_msg = f'[AUTOSTEER] Error in automation cycle: {str(e)}'
            self.logger.error_detailed(error_msg, f'Full traceback:\n{traceback.format_exc()}')
            ...
            if screenshot_path:
                try:
                    ...
                    self.logger.warning(f'[AUTOSTEER] Failed to delete screenshot {screenshot_path}: {cleanup_error}')
