    ...
    def execute_cycle(self):
        ...
        try:
            ...
            if data.throttle is not None and data.throttle == 0:
                ...
                if phase not in _DOCKING_PHASES:
                    self.logger.info(f'[AUTOSTEER] Throttle at 0% - holding W for {THROTTLE_ZERO_HOLD_W_S} seconds')
                    self.keyboard.release_all()
                    self.keyboard.hold_w(THROTTLE_ZERO_HOLD_W_S, lambda: not self.running)
