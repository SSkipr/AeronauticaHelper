...
class AutoSteerMode(MonitoringMode):
    ...
    def __init__(self, cycle_interval, webhook_url, multiplier, logger, human_detector=None, custom_waypoint=False, include_screenshots=False, keyboard=None, throttle_up_if_not_100=False):
    ...
    def _should_skip_throttle_up(self):
        ...
        return phase in _SKIP_THROTTLE_UP_PHASES
    ...
    def execute_cycle(self):
        ...
        try:
            ...
            data = self.parser.parse(text, preferred_code=preferred, prefer_custom=prefer_custom)
            if hasattr(self, '_filter_icao_for_autopilot') and callable(getattr(self, '_filter_icao_for_autopilot')):
            ...
            if self.override_icao_code == 'UNDOCK':
                ...
                if urgent:
                    ...
                    self.notifier.send_urgent_alert(data, self.previous_distance, autosteer_enabled, mode=mode, override_target_bearing=self.override_target_bearing, override_icao_code=self.override_icao_code, throttle_up_if_not_100=self.throttle_up_if_not_100, will_throttle_up=will_throttle_up)
                else:
