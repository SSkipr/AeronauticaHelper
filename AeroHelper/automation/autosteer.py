...
STUCK_DISTANCE_EPS_NM = 0.02
STUCK_DISTANCE_CYCLES = 3
STUCK_RECOVERY_MIN_DISTANCE_NM = 1.5
STUCK_AD_PHASE_S = 200.0
STUCK_AD_SEGMENT_S = 10.0
STUCK_W_AFTER_S = 7.0
...
class AutoSteerMode(MonitoringMode):
    ...
    def __init__(self, cycle_interval, webhook_url, multiplier, logger, human_detector=None, custom_waypoint=False, include_screenshots=False, keyboard=None):
        ...
        self._stagnation_ref_distance = None
    ...
    def _stuck_distance_recovery_active(self):
        return True

    def _sleep_chunked(self, duration, stop_check, chunk=0.25):
        if duration <= 0 or stop_check():
            return
        elapsed = 0.0
        while elapsed < duration and (not stop_check()):
            step = min(chunk, duration - elapsed)
            time.sleep(step)
            elapsed += step
    ...
    def _run_stuck_distance_recovery(self):
        stop = lambda: not self.running
        self.logger.info(f'[AUTOSTEER] Stuck recovery: hold Z for up to {STUCK_AD_PHASE_S:.0f}s with A/D alternation ({STUCK_AD_SEGMENT_S:.0f}s per segment)')
        self.keyboard.press('z')
        t0 = time.perf_counter()
        try:
            while time.perf_counter() - t0 < STUCK_AD_PHASE_S and (not stop()):
                rem = STUCK_AD_PHASE_S - (time.perf_counter() - t0)
                self.keyboard.press('a')
                self._sleep_chunked(min(STUCK_AD_SEGMENT_S, rem), stop)
                self.keyboard.release('a')
                if time.perf_counter() - t0 >= STUCK_AD_PHASE_S or stop():
                    break
                rem = STUCK_AD_PHASE_S - (time.perf_counter() - t0)
    ...
    def execute_cycle(self):
        ...
        try:
            ...
            if not self._stuck_distance_recovery_active():
                self._stagnation_ref_distance = None
                self._stagnation_cycles = 0
            elif data.distance is not None and data.distance >= STUCK_RECOVERY_MIN_DISTANCE_NM:
                if self._stagnation_ref_distance is None:
                    self._stagnation_ref_distance = data.distance
                    self._stagnation_cycles = 1
                elif abs(data.distance - self._stagnation_ref_distance) <= STUCK_DISTANCE_EPS_NM:
                    self._stagnation_cycles += 1
                else:
                    self._stagnation_ref_distance = data.distance
                    self._stagnation_cycles = 1
                if self._stagnation_cycles >= STUCK_DISTANCE_CYCLES:
                    self.logger.warning(f'[AUTOSTEER] Distance ~{data.distance} nm unchanged {STUCK_DISTANCE_CYCLES} cycles - stuck recovery (Z + A/D alternation up to 200s; then W 7s)')
                    try:
                        self.notifier.send_warning('AutoSteer', 'Distance unchanged 3 cycles; Z hold with A/D alternation (200s max), then W 7s.', ping=False)
                    except Exception:
                        pass
                    self._stagnation_ref_distance = None
                    self._stagnation_cycles = 0
                    self._run_stuck_distance_recovery()
                    self.keyboard.hold_w(STUCK_W_AFTER_S, lambda: not self.running)
