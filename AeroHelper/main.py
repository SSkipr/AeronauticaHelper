...
from AeroHelper.ui.startup_countdown import StartupCountdownOverlay
...
def main():
    ...
    startup_countdown = StartupCountdownOverlay()
    controller = Controller(main_window, status_overlay, app_version=AEROHELPER_VERSION_DEV, startup_countdown=startup_countdown)
    main_window.show()
    status_overlay.show()

    def _apply_win_taskbar_icons():
        if not app_icon.isNull():
            apply_windows_taskbar_icons([main_window, status_overlay, startup_countdown], app_icon)
