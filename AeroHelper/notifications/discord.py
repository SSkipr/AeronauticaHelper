...
from AeroHelper.utils.bearing import shortest_angle_diff_abs
...
class DiscordNotifier:
    ...
    def __init__(self, webhook_url=None, logger=None, app_version=None, policy=None):
    ...
    def _policy(self):
        ...
        if policy is None:
            from AeroHelper.notifications.policy import NotificationPolicy
            policy = NotificationPolicy()
            self.policy = policy
        return policy

    def _notify_allowed(self, category):
        if not self.webhook_url:
            if self.logger:
                self.logger.debug(f'Discord webhook URL not configured - skipping {category}')
            return False
        if not self._policy().should_send(category):
            if self.logger:
                self.logger.debug(f'[WEBHOOK] Skipping {category} (notification mode: {self._policy().mode})')
            return False
        return True

    def _notify_content(self, category, extra=''):
        ping = self._policy().should_ping(category)
        extra = (extra or '').strip()
        if ping and extra:
            return f'@everyone {extra}'
        if ping:
            return '@everyone'
        return extra
    ...
    def send_status_update(self, data, previous_distance=None, screenshot_path=None, cycle_duration_sec=None, mode=None, override_target_bearing=None, override_icao_code=None, phase=None, autopilot_multiplier=None):
        if not self._notify_allowed('status'):
            return
        try:
            ...
            log_msg = f'[WEBHOOK] Sending STATUS UPDATE: Speed={spd}, Throttle={data.throttle}%, Fuel={data.fuel}%, Distance={self._fmt_distance(data.distance)}, HDG={data.heading}°, Target={target_str} ({icao_code}), ETA={arrival_str}, TimeRemaining={time_remaining_str}'
            if screenshot_path:
            ...
            has_screenshot = self._has_screenshot(screenshot_path)
            embed = self._embed(title, COLOR_INFO, fields=[self._field('Speed', f'{data.speed} Knots' if data.speed is not None else 'N/A'), self._field('Throttle', f'{data.throttle}%' if data.throttle is not None else 'N/A'), self._field('Fuel', f'{data.fuel}%' if data.fuel is not None else 'N/A'), self._field('Distance', self._fmt_distance(data.distance)), self._field('Heading', f'{data.heading}°' if data.heading is not None else 'N/A'), self._field('Target', f'{int(round(target_bearing))}° ({icao_code})' if target_bearing is not None else 'N/A'), self._field('ETA', arrival_str), self._field('Time Left', time_remaining_str)])
            if cycle_duration_sec is not None:
                embed['fields'].append(self._field('Cycle', f'{cycle_duration_sec:.2f}s'))
            if has_screenshot:
                embed['image'] = {'url': 'attachment://screenshot.png'}
            payload = {'content': self._notify_content('status'), 'embeds': [embed]}
            if self.logger:
                self.logger.debug(f'[WEBHOOK] Sending STATUS UPDATE (ping={self._policy().should_ping("status")})')
    ...
    def send_urgent_alert(self, data, previous_distance=None, autosteer_enabled=False, mode=None, override_target_bearing=None, override_icao_code=None, throttle_up_if_not_100=False, will_throttle_up=False, stale_distance=False):
        if not self._notify_allowed('urgent'):
            return
        try:
            target_bearing = override_target_bearing if override_target_bearing is not None else data.target_bearing
            icao_code = override_icao_code if override_icao_code is not None else data.icao_code
            ...
            if data.heading is not None and target_bearing is not None:
                hdg_diff = shortest_angle_diff_abs(target_bearing, data.heading)
                if hdg_diff >= 5:
            ...
            if data.throttle is not None and data.throttle < 100:
                setting_state = 'ON' if throttle_up_if_not_100 else 'OFF'
                if will_throttle_up:
                    urgent_reasons.append(
                        f'Throttle = {data.throttle}%. Throttle-up setting: {setting_state} - holding W for 10s to restore full throttle'
                    )
                elif data.throttle == 0:
                    urgent_reasons.append(
                        f'Throttle = {data.throttle}%. Throttle-up setting: {setting_state} - no action'
                    )
            if stale_distance and previous_distance is not None and data.distance is not None:
                distance_diff = abs(data.distance - previous_distance)
                if distance_diff < 0.01:
                    urgent_reasons.append(f'Distance unchanged (prev: {self._fmt_distance(previous_distance)}, curr: {self._fmt_distance(data.distance)}). This may be a false positive, if this continues, something is wrong.')
            ...
            embed = self._embed(title, COLOR_ERROR, description='\n'.join(description_parts), fields=[self._field('Heading', f'{data.heading}°' if data.heading is not None else 'N/A'), self._field('Target', f'{int(round(target_bearing))}° ({icao_code})' if target_bearing is not None else 'N/A'), self._field('Fuel', f'{data.fuel}%' if data.fuel is not None else 'N/A'), self._field('Throttle', f'{data.throttle}%' if data.throttle is not None else 'N/A'), self._field('Distance', self._fmt_distance(data.distance))])
            payload = {'content': self._notify_content('urgent'), 'embeds': [embed]}
            if self.logger:
                self.logger.debug(f'[WEBHOOK] Sending URGENT ALERT (ping={self._policy().should_ping("urgent")})')
            response = requests.post(self.webhook_url, json=payload, timeout=10)
            if response.status_code in (200, 204):
                if self.logger:
    ...
    def send_steering_correction(self, data, direction, duration, diff, override_target_bearing=None, override_icao_code=None, mode=None):
        if not self._notify_allowed('steering'):
            return
        try:
            ...
            payload = {'content': self._notify_content('steering'), 'embeds': [embed]}
            response = requests.post(self.webhook_url, json=payload, timeout=10)
            if response.status_code in (200, 204):
                if self.logger:
    ...
    def send_oscillation_alert(self, data, multiplier):
        ...
        try:
            ...
            payload = {'content': self._notify_content('urgent'), 'embeds': [embed]}
            if self.logger:
                self.logger.debug(f'[WEBHOOK] Sending OSCILLATION ALERT (ping={self._policy().should_ping("urgent")})')
            response = requests.post(self.webhook_url, json=payload, timeout=10)
            if response.status_code in (200, 204):
                if self.logger:
    ...
    def send_reconnect_update(self, stage, message, screenshot_path=None):
        if not self._notify_allowed('disconnect'):
            return
        try:
            ...
            ping = self._policy().should_ping('disconnect') and str(stage) in ping_stages
    ...
    def send_undocking_status(self, data):
        if not self._notify_allowed('undocking'):
            return
        try:
            if self.logger:
            ...
            payload = {'content': self._notify_content('undocking'), 'embeds': [embed]}
            response = requests.post(self.webhook_url, json=payload, timeout=10)
            if response.status_code in (200, 204):
                if self.logger:
    ...
    def send_autopilot_update(self, phase, message, details=None, ping=False, screenshot_path=None, embed_color=None):
        category = 'stop_quit' if ping else 'status'
        if not self._notify_allowed(category):
            return
        try:
            if self.logger:
            ...
            if has_screenshot:
                embed['image'] = {'url': 'attachment://screenshot.png'}
            payload = {'content': self._notify_content(category), 'embeds': [embed]}
    ...
    def send_mission_complete(self, destination, wp=None, money=None, mission_number=None):
        if not self._notify_allowed('mission_complete'):
            return
        try:
            if self.logger:
            ...
            payload = {'content': self._notify_content('mission_complete', '**Job done!** Another mission in the bag 🎊'), 'embeds': [embed]}
            response = requests.post(self.webhook_url, json=payload, timeout=10)
            if response.status_code in (200, 204):
                if self.logger:
    ...
    def send_destination_reached(self, destination=None, distance=None, mode='AutoSteer'):
        if not self._notify_allowed('mission_complete'):
            return
        try:
            dest = destination or 'DEST'
            dist_txt = self._fmt_distance(distance) if distance is not None else 'under threshold'
            if self.logger:
                self.logger.info(f'[WEBHOOK] Sending destination reached: mode={mode}, dest={dest}, distance={dist_txt}')
            description = (
                f'**You\'re at the destination!** ✈️\n\n'
                f'{mode} stopped near **{dest}** ({dist_txt}).\n\n'
                f'Dock manually when ready — automation will stay stopped.'
            )
            fields = [
                {'name': '📍 Target', 'value': str(dest), 'inline': True},
                {'name': '📏 Distance', 'value': dist_txt, 'inline': True},
                {'name': '🧭 Mode', 'value': str(mode), 'inline': True},
            ]
            embed = self._embed('🎊 Destination Reached!', COLOR_SUCCESS, description=description, fields=fields)
            payload = {'content': self._notify_content('mission_complete', '**Destination reached!** Time to dock 🎊'), 'embeds': [embed]}
            response = requests.post(self.webhook_url, json=payload, timeout=10)
            if response.status_code in (200, 204):
                if self.logger:
                    self.logger.info('[WEBHOOK] Destination reached sent successfully')
                return True
            if self.logger:
                self.logger.error(f'[WEBHOOK] Failed to send destination reached (status: {response.status_code})')
            return False
        except REQUEST_EXCEPTIONS as e:
            if self.logger:
                self.logger.info(f'[WEBHOOK] No network - skipping destination reached: {str(e)}')
            return False
        except Exception as e:
            if self.logger:
                self.logger.error(f'[WEBHOOK] Exception sending destination reached: {str(e)}')
            return False

    def send_warning(self, title, message, ping=True):
        category = 'stop_quit' if ping else 'warnings'
        if not self._notify_allowed(category):
            return
        try:
            if self.logger:
    ...
    def send_quit(self, reason):
        if not self._notify_allowed('stop_quit'):
            return
        try:
            ...
            payload = {'content': self._notify_content('stop_quit'), 'embeds': [embed]}
            response = requests.post(self.webhook_url, json=payload, timeout=10)
