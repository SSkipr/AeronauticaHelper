...
class DiscordNotifier:
    ...
    def send_urgent_alert(self, data, previous_distance=None, autosteer_enabled=False, mode=None, override_target_bearing=None, override_icao_code=None):
        if not self.webhook_url:
            if self.logger:
        ...
        try:
            ...
            if data.throttle is not None and data.throttle == 0:
                urgent_reasons.append('Throttle = 0%')
            if previous_distance is not None and data.distance is not None:
                distance_diff = abs(data.distance - previous_distance)
                if distance_diff < 0.01:
                    urgent_reasons.append(f'Distance unchanged (prev: {previous_distance}nm, curr: {data.distance}nm). This may be a false positive, if this continues, something is wrong. AeroHelper will attempt to fix shortly.')
