...
class DiscordNotifier:
    ...
    def send_destination_reached(self, destination=None, distance=None, mode='AutoSteer'):
        if not self.webhook_url:
            if self.logger:
                self.logger.debug('[WEBHOOK] Discord webhook URL not configured - skipping destination reached')
            return
        try:
            ...
            payload = {'content': '@everyone **Destination reached!** Time to dock 🎊', 'embeds': [embed]}
            response = requests.post(self.webhook_url, json=payload, timeout=10)
            if response.status_code in (200, 204):
                if self.logger:
