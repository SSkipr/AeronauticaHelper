...
from AeroHelper.utils.halt import interruptible_sleep, is_halted
...
class MonitoringMode:
    ...
    def _sleep_if_running(self, seconds):
        if not self.running or is_halted():
            return False
        return interruptible_sleep(seconds) and self.running

    THROTTLE_UP_HOLD_W_S = 10

    def __init__(self, cycle_interval, webhook_url, logger, human_detector=None, notification_mode='all', custom_waypoint=False, include_screenshots=False, keyboard=None, skip_current_bearing=False, throttle_up_if_not_100=False):
        ...
        self.skip_current_bearing = skip_current_bearing
        self.throttle_up_if_not_100 = throttle_up_if_not_100
        self._distance_growing_count = 0
        self._unchanged_distance_cycles = 0

    def _click_chat_close(self, screenshot_path):
        try:
            found_boxes = self.ocr.find_text_boxes(
                screenshot_path, 'hide', match_mode='word', prefer_windows=True
            )
            if not found_boxes:
            ...
            for i, box_data in enumerate(found_boxes):
                box_coords = box_data.get('box') or []
                if len(box_coords) < 4:
                    self.logger.warning(f"[CHAT CLOSE] Invalid bounding box format for '{box_data.get('text')}': {box_coords}")
                    continue
    ...
    def execute_cycle(self):
        ...
        try:
            try:
                ...
                self.logger.warning(f'[MONITORING] Primary screenshot capture failed, trying PIL fallback: {capture_error}')
                try:
                    from AeroHelper.utils.screenshot import capture_screen_pil
                    screenshot_path = capture_screen_pil()
                    self.logger.info('[MONITORING] Successfully used PIL screenshot fallback')
                except Exception as pil_error:
                    import traceback
                    error_msg = f'[MONITORING] Screenshot capture failed completely. Primary error: {capture_error}. PIL error: {pil_error}'
            ...
            if self.ocr.detect_return_to_lobby(text):
                self.logger.warning('[MONITORING] Return to Lobby detected - pausing')
                return {'action': 'pause', 'reason': 'Return to Lobby detected'}
            if self.ocr.detect_disconnected(text):
                self.logger.warning('[MONITORING] Disconnected detected - entering reconnect mode')
            ...
            if self.skip_current_bearing:
                data.heading = None
            self.logger.log_parsed(data.to_dict())
            if not data.valid:
                self.logger.warning(
                    '[MONITORING] Invalid game state - distance (nm/km) or speed (knots/km/h) not found. '
                    'Join the Aeronautica Roblox game and be mid-mission with the HUD visible '
                    '(not lobby, not another experience, no overlay covering Roblox).'
                )
                return {'action': 'pause', 'reason': 'Invalid game state'}
            if (
                not self.custom_waypoint
                and data.icao_code is not None
                and data.icao_code.upper() != 'DEST'
            ):
                self.logger.warning(f'[MONITORING] Waypoint detected (ICAO: {data.icao_code}) - pausing to avoid distance calculation errors')
            ...
            if not self.skip_current_bearing:
                self.keyboard.tap('5')
            else:
                self.logger.debug('[MONITORING] Skip current bearing - not pressing 5')
            ...
            screenshot_sent = bool(scr)
            if urgent:
                self.notifier.send_urgent_alert(data, self.previous_distance, autosteer_enabled, mode='Monitoring', throttle_up_if_not_100=self.throttle_up_if_not_100, will_throttle_up=will_throttle_up, stale_distance=self._unchanged_distance_cycles >= 2)
            ...
            if screenshot_path and screenshot_sent:
                try:
                    ...
                    self.logger.warning(f'[MONITORING] Failed to delete screenshot {screenshot_path}: {cleanup_error}')
            return {'action': 'continue', 'data': data}
        except Exception as e:
            import traceback
            error_msg = f'[MONITORING] Error in monitoring cycle: {str(e)}'
            self.logger.error_detailed(error_msg, f'Full traceback:\n{traceback.format_exc()}')
    ...
    def _throttle_below_100(self, data):
        return data.throttle is not None and data.throttle < 100

    def _should_skip_throttle_up(self, data=None):
        return False

    def _will_throttle_up(self, data):
        return bool(self.throttle_up_if_not_100 and self._throttle_below_100(data) and (not self._should_skip_throttle_up(data)))

    def _maybe_throttle_up(self, data):
        if not self._will_throttle_up(data):
            return
        hold_s = self.THROTTLE_UP_HOLD_W_S
        self.logger.info(f'[THROTTLE] Throttle at {data.throttle}% (not 100%) - holding W for {hold_s}s')
        self.keyboard.release_all()
        self.keyboard.hold_w(hold_s, lambda: not self.running)

    def _is_throttle_urgent(self, data):
        if not self._throttle_below_100(data) or self._should_skip_throttle_up(data):
            return False

        return self.throttle_up_if_not_100 or data.throttle == 0
    ...
    def _is_urgent(self, data):
        if self.previous_distance is not None and data.distance is not None:
            if abs(data.distance - self.previous_distance) < 0.01:
                self._unchanged_distance_cycles += 1
            else:
                self._unchanged_distance_cycles = 0
        else:
            self._unchanged_distance_cycles = 0
        if data.heading is not None and data.target_bearing is not None:
            if shortest_angle_diff_abs(data.target_bearing, data.heading) >= 5:
                return True
        if data.fuel is not None and data.fuel < 10:
            return True
        if self._is_throttle_urgent(data):
            return True
        return self._unchanged_distance_cycles >= 2
