...
from AeroHelper.utils.halt import clear_halt, halt_scope, interruptible_sleep, is_halted, request_halt, wait_for_halt
...
_OCR_DUMP_MARKERS = (
    ...
    'Windows OCR: Processing took',
    'Vision OCR: Processing took',
    '] OCR:',
)
_OCR_BLOB_MARKERS = (
    'OCR Raw Text:',
...
def _is_timestamped_log_line(line):
    return line.startswith('[') and '] [' in line[:80]

def _ocr_blob_from_line(line):
    for marker in _OCR_BLOB_MARKERS:
        ...
        if rest:
            return rest
    for prefix in ('Windows OCR: Processing took', 'Vision OCR: Processing took'):
...
def _sanitize_log_tail_for_datashare(log_tail):
    ...
    last_ocr = None
    skipping = False
    collecting_combined = False
    combined_parts = []

    def _finish_combined():
        nonlocal last_ocr, collecting_combined, combined_parts
        if combined_parts:
    ...
    for line in lines:
        if skipping:
            if not _is_timestamped_log_line(line):
                stripped = line.strip()
                if stripped.startswith('--- combined'):
                    _finish_combined()
                    collecting_combined = True
                    combined_parts = []
                elif collecting_combined:
            ...
            _finish_combined()
            skipping = False
        if any(marker in line for marker in _OCR_DUMP_MARKERS):
            skipping = True
            collecting_combined = False
            combined_parts = []
            blob = _ocr_blob_from_line(line)
            if blob:
                last_ocr = blob
            continue
        out.append(line)
...
class Controller:
    ...
    def __init__(self, main_window, status_overlay, app_version=None, startup_countdown=None):
        ...
        self.config = Config()
        self.logger = Logger()
        self.logger.ocr_debug_enabled = self.config.get_ocr_debug()
        ...
        self.main_window.ocr_debug_changed.connect(self._on_ocr_debug_changed)

    def _on_ocr_debug_changed(self, enabled):
        self.logger.ocr_debug_enabled = bool(enabled)
        self.logger.info(f'[OCR-DEBUG] {"on" if enabled else "off"}')
    ...
    def _sleep_with_running_check(self, seconds):
        if not self.running or is_halted():
            return
    ...
    def _run_startup_countdown(self, seconds=10):
        ...
        if overlay is None:
            ...
            return bool(self.running and not self._startup_cancelled() and not is_halted())
        ...
        while not done.is_set():
            if is_halted() or not self.running or self._startup_cancelled():
    ...
    def _build_diagnostics(self, data=None):
        ...
        if data is not None:
            ...
            diagnostics['Distance'] = f'{data.distance:.2f} nm' if data.distance is not None else 'N/A'
    ...
    def _format_ship_status_line(self):
        ...
        if isinstance(ship, dict):
            ...
            if ship.get('distance_nm') is not None:
                parts.append(f"{float(ship['distance_nm']):.2f} nm")
        ...
        if details.get('distance') is not None:
            parts.append(f"{float(details['distance']):.2f} nm")
    ...
    def _create_automation_mode(self):
        ...
        intelligent_steering = self.config.get_intelligent_steering()
        ...
        if mode_str == 'Monitoring':
            ...
            mode = MonitoringMode(cycle_interval, webhook_url, self.logger, self.human_detector, notification_mode, custom_waypoint=custom_waypoint, include_screenshots=include_screenshots, keyboard=shared_keyboard, skip_current_bearing=skip_current_bearing, throttle_up_if_not_100=throttle_up)
            self._attach_notification_policy(mode)
            return (mode, mode_str, cycle_interval, multiplier, notification_mode, webhook_url, include_screenshots, custom_waypoint)
        if mode_str == 'AutoSteer':
            custom_waypoint = self.config.get_autosteer_custom_waypoint()
            mode = AutoSteerMode(cycle_interval, webhook_url, multiplier, self.logger, self.human_detector, custom_waypoint=custom_waypoint, include_screenshots=include_screenshots, keyboard=shared_keyboard, throttle_up_if_not_100=throttle_up, intelligent_steering=intelligent_steering)
            self._attach_multiplier_persist(mode)
            self._attach_notification_policy(mode)
            return (mode, mode_str, cycle_interval, multiplier, notification_mode, webhook_url, include_screenshots, custom_waypoint)
        if mode_str == 'AutoPilot':
            start_mid_mission = self.config.get_start_mid_mission()
            mode = AutoPilotMode(cycle_interval, webhook_url, multiplier, self.logger, self.human_detector, start_mid_mission=start_mid_mission, include_screenshots=include_screenshots, history_callback=self.add_history_entry, keyboard=shared_keyboard, throttle_up_if_not_100=throttle_up, intelligent_steering=intelligent_steering)
            self._attach_multiplier_persist(mode)
            self._attach_notification_policy(mode)
            return (mode, mode_str, cycle_interval, multiplier, notification_mode, webhook_url, include_screenshots, None)
        raise ValueError(f'Unknown mode: {mode_str}')

    def _attach_notification_policy(self, mode):
        policy = self.config.build_notification_policy()
        if mode is not None and getattr(mode, 'notifier', None) is not None:
            mode.notifier.policy = policy
        if getattr(self, 'reconnect_handler', None) is not None and getattr(self.reconnect_handler, 'notifier', None) is not None:
            self.reconnect_handler.notifier.policy = policy

    def _attach_multiplier_persist(self, mode):
        mode.multiplier_persist_callback = self._persist_learned_multiplier

    def _persist_learned_multiplier(self, value):
        try:
            self.main_window.update_multiplier_signal.emit(float(value))
        except Exception:
            try:
                self.config.set_multiplier(float(value))
            except Exception:
                pass
    ...
    def _log_session_start(self, mode_str, cycle_interval, multiplier, notification_mode, webhook_url, include_screenshots, custom_waypoint=None):
        ...
        if mode_str in ('AutoSteer', 'AutoPilot'):
            intelligent = self.config.get_intelligent_steering()
            lines.insert(5, f'[SESSION] Intelligent steering: {intelligent}' + (' (beta; persists learned multiplier)' if intelligent else ''))
    ...
    def start(self):
        ...
        try:
            ...
            self._reset_start_ui(AutomationState.ERROR)
            return
        leftover = self.automation_thread
        if leftover is not None and leftover.is_alive():
            self.logger.info('[STARTUP] Previous automation thread still alive - waiting for it to finish')
            request_halt()
            self.running = False
            if self.automation_mode:
                try:
                    ...
                    pass
            self._join_automation_thread(timeout=25)
        self.logger.info('[STARTUP] Starting AeroHelper')
        self._start_cancelled = False
        if IS_MACOS:
    ...
    def _start_automation_worker_impl(self):
        ...
        from AeroHelper.notifications.policy import mode_label
        notification_label = mode_label(notification_mode)
        ...
        if focus_ok:
            self.logger.info('[STARTUP] Brought Roblox window to front')
            time.sleep(0.3)
            if needs_f11:
                self.logger.info('[STARTUP] Roblox not F11 fullscreen, pressing F11')
                self.keyboard.press(Key.f11)
                self.keyboard.release(Key.f11)
                time.sleep(0.5)
            else:
                self.logger.info('[STARTUP] Roblox already F11 fullscreen')
        else:
            self.logger.warning('[STARTUP] Failed to bring Roblox window to front')
            if IS_MACOS:
        ...
        def delayed_start():
            with halt_scope():
                ...
                try:
                    ...
                    pass
                while self.running and self.paused and not is_halted():
                    wait_for_halt(0.1)
                if not completed or not self.running or self._startup_cancelled() or is_halted():
                    self.logger.info('[STARTUP] Start cancelled during countdown')
                    self._hide_startup_countdown()
                    return
                if self.running and self._run_generation == run_gen:
                    ...
                    self._run_automation_loop(run_gen)
    ...
    def _start_intervention_monitoring(self):
        ...
        def monitor_loop():
            ...
            while self.intervention_monitoring and self.running:
                ...
                if self.human_detector.check_intervention():
                    self.logger.info(f'[HUMAN] Intervention detected - current state: {self.state}, paused: {self.paused}')
                    if self.state != AutomationState.PAUSED_HUMAN:
                        self.logger.info('[HUMAN] Calling _handle_human_intervention()')
                        self._handle_human_intervention()
                    else:
                        self.logger.info('[HUMAN] Already in PAUSED_HUMAN state - resetting countdown')
                        self.retry_countdown = 10
                        if self.countdown_running:
                            self.logger.info('[HUMAN] Human input detected while paused - resetting countdown to 10 seconds')
                        else:
        ...
        self.logger.info('[HUMAN] Started dedicated human intervention monitoring thread (checks every 0.1 seconds)')

    def _join_automation_thread(self, timeout=25):
        thread = self.automation_thread
        if thread is None or not thread.is_alive():
            self.automation_thread = None
            return True
        if thread is threading.current_thread():
            self.logger.info('[STOP] Stop called from automation thread - skipping join')
            return True
        self.logger.info(f'[STOP] Waiting for automation thread to finish (timeout={timeout}s)')
        thread.join(timeout=timeout)
        if thread.is_alive():
            self.logger.warning('[STOP] Automation thread still running after join timeout')
            return False
        self.logger.info('[STOP] Automation thread finished')
        self.automation_thread = None
        return True

    def stop(self, clear_error=True):
        try:
            self.logger.info('[STOP] Stopping automation...')
            request_halt()
            ...
            try:
                ...
                self.logger.error(f'[STOP] Error releasing keyboard keys: {e}')
            if self.automation_mode:
                try:
                    self.automation_mode.stop()
                except Exception as e:
                    self.logger.error(f'[STOP] Error stopping automation mode: {e}')
            if self._join_automation_thread(timeout=25):
                clear_halt()
            try:
                self.human_detector.unsuspend()
            except Exception:
            ...
            try:
                ...
                self.logger.error(f'[STOP] Error stopping human detector: {e}')
            self._update_status_overlay(self.state)
            if clear_error:
            ...
            try:
                ...
                self.logger.error(f'[STOP] Error updating UI: {e}')
            self.logger.info('[STOP] Automation stopped successfully')
        except Exception as e:
            import traceback
            self.logger.error(f'[STOP] Error in stop method: {e}')
            self.logger.error_detailed('Stop method error', traceback.format_exc())

    def _run_automation_loop(self, run_gen=None):
        while self.running:
            if run_gen is not None and run_gen != self._run_generation:
                self.logger.info('[AUTOMATION] Stale run generation - exiting loop')
                return
            ...
            try:
                ...
                if not self.running or (run_gen is not None and run_gen != self._run_generation):
                    return
                if result is None:
                ...
                if action == 'pause':
                    ...
                    if reason == 'Distance threshold reached':
                        self.logger.info(f'[AUTOMATION] Cycle pause: {reason} (destination reached)')
                        self._handle_destination_reached(result)
                        return
                    self.logger.warning(f'[AUTOMATION] Cycle pause: {reason}')
                    ...
                    self.logger.activate_error_ocr_dump()
                    self._emit_main_status('error', f'Error: {error}', self._build_diagnostics(result.get('data')))
                    self.logger.error(f'[AUTOMATION] Error in automation: {error}')
                    self.consecutive_error_count += 1
                    if self.config.get_quit_after_5_errors():
                        if self.consecutive_error_count >= 5:
                ...
                self.logger.activate_error_ocr_dump()
                self.logger.error(f'[AUTOMATION] Exception in automation loop: {e}')
                if self.config.get_quit_after_5_errors():
                    self.consecutive_error_count += 1
                    if self.consecutive_error_count >= 5:
    ...
    def _post_datashare(self, error_summary, *, manual_override=False, on_complete=None):
        ...
        def _send():
            ...
            try:
                ...
                post_api('/api/datashare', body, config=self.config, app_version=self.app_version, logger=self.logger)
    ...
    def _handle_human_intervention(self):
        self.logger.info('[HUMAN] Human intervention detected - pausing automation')
        self.paused = True
    ...
    def _start_retry_countdown(self):
        ...
        def countdown():
            while self.retry_countdown > 0 and self.paused and self.running and (self.state == AutomationState.PAUSED_HUMAN):
                if self.human_detector.check_intervention():
                    self.retry_countdown = 10
                    self.logger.info('[HUMAN] Human input detected during countdown - resetting to 10 seconds')
                else:
                    self.retry_countdown -= 1
                    if self.retry_countdown > 0:
                        self.logger.info(f'[HUMAN] Resume countdown: {self.retry_countdown} seconds')
                wait_for_halt(1)
            if self.retry_countdown == 0 and self.paused and self.running and (self.state == AutomationState.PAUSED_HUMAN):
                self.logger.info('[HUMAN] Resume countdown finished - resuming automation')
                self.paused = False
                self.state = AutomationState.RUNNING
                self._update_status_overlay(self.state)
                self._emit_main_status('ok', self._automation_status_message(), self._build_diagnostics())
    ...
    def _handle_destination_reached(self, result=None):
        data = (result or {}).get('data')
        if data is not None:
            self._last_cycle_data = data
        mode = self._session_mode or self.config.get_mode() or 'AutoSteer'
        distance = getattr(data, 'distance', None) if data is not None else None
        dest = None
        if data is not None:
            dest = getattr(data, 'icao_code', None) or getattr(data, 'destination', None)
        dest = dest or 'DEST'
        dist_label = f'{float(distance):.2f} nm' if distance is not None else 'under stop threshold'
        self.paused = True
        self.state = AutomationState.PAUSED
        self._update_status_overlay(self.state)
        self._emit_main_status('ok', f'Destination reached ({dist_label}) — dock manually', self._build_diagnostics(data))
        self._record_session_history('Destination Reached', f'Stopped near {dest} ({dist_label})')
        try:
            self.automation_mode.notifier.send_destination_reached(destination=dest, distance=distance, mode=mode)
        except Exception as e:
            self.logger.error(f'[WEBHOOK] Failed to send destination-reached webhook: {e}')
        title = 'Destination Reached'
        message = (
            f'AeroHelper stopped near the destination ({dest}, {dist_label}).\n\n'
            'Please dock the vehicle manually.\n'
            'You can start a new mission afterward, or resume AutoPilot from the lobby.'
        )
        self.main_window.show_error_signal.emit(title, message, '')
        self.main_window.is_running = False
    ...
    def _handle_pause_with_error(self, reason, details=''):
        ...
        try:
            ...
            self.logger.error(f'[WEBHOOK] Failed to send quit webhook: {e}')
    ...
    def _handle_5_consecutive_errors(self, last_error):
        self.logger.activate_error_ocr_dump()
        self.logger.error(f'[AUTOMATION] 5 consecutive errors - stopping and sending @everyone to Discord. Last error: {last_error}')
        ...
        try:
            ...
            self.logger.error(f'[WEBHOOK] Failed to send 5-errors webhook: {e}')
    ...
    def _handle_reconnect(self):
        ...
        self.logger.info('[RECONNECT] Entering reconnection mode')
        success = self.reconnect_handler.execute_reconnect(stop_check=lambda: not self.running)
        if not self.running:
            self.logger.info('[RECONNECT] Reconnection aborted - automation stopped')
            return
        if success:
            self.logger.info('[RECONNECT] Reconnection successful - resuming automation')
            self.state = AutomationState.RUNNING
            self._update_status_overlay(self.state)
            self._emit_main_status('ok', self._automation_status_message(), self._build_diagnostics())
            self.paused = False
        else:
            self.logger.error('[RECONNECT] Reconnection failed - stopping')
            self.logger.activate_error_ocr_dump()
            self.paused = True
            self.state = AutomationState.PAUSED
            self._update_status_overlay(self.state)
