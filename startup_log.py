...
def _base_fields():
    return {
        ...
        "os": f"{platform.system()} {get_os_display_name()}".strip(),
...
def log_startup_banner(logger, app_version, *, extra=None):
    if not logger:
        return
    fields = {"version": app_version, **_base_fields()}
    if extra:
        fields.update(extra)
    lines = ["[AEROHELPER STARTUP]"]
